...
| Icônes PWA (installation écran d'accueil, splash Android, favicon) | ~~`public/logos/solid-mark-source.png` généré via MCP Higgsfield (anneaux entrelacés)~~ → **remplacé par le logo officiel fourni par l'utilisateur** (juillet 2026) : cercle de 8 points verts type "spinner", fond blanc plein cadre, extrait/nettoyé depuis le fichier fourni (suppression du fond de mockup gris et de l'ombre portée). Dérivés régénérés depuis ce fichier : `public/icons/icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, `apple-touch-icon.png`, `src/app/favicon.ico`. | ✅ Résolu | L'exception ponctuelle à la règle "jamais généré par IA" (notée précédemment ici) n'a plus lieu d'être : ce logo est désormais un fichier fourni par l'utilisateur, conforme à la règle CLAUDE.md standard. |
